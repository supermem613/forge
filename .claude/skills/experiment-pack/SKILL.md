...
```text
forge pack-analysis <experiment> --to <out.zip>
forge verify-analysis <out.zip>
```

## Policy

- Do not invent file lists or copy paths by hand. The CLI owns selection, runbook snapshot, hashing, `ANALYSIS.json`, and zip creation.
...
- After packing, run `forge verify-analysis` when the user wants confirmation the zip is intact.
