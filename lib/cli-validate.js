...
import { resolveEvalKind } from './judge-orchestrator.js';

const SEMVER_RE = /^\d+\.\d+\.\d+$/;
const STEP_SHIM_WARN_LINES = 120;
const RUN_STEP_WARN_LINES = 300;
const CRITERION_MIN_CHARS = 8;
// Long enough to force "behavior + how must proves it", not a one-word label.
// Measured against scaffold/example strings in lib/cli-new-runbook.js (~45+ chars).
const EVAL_DESCRIPTION_MIN_CHARS = 24;
...
export async function validateRunbook({ runbookDir, runbookId } = {}) {
  ...
  if (!Array.isArray(manifest.evals) || manifest.evals.length === 0) {
    ...
    for (const rel of manifest.evals) {
      ...
      const description = typeof evalJson.description === 'string'
        ? evalJson.description.trim()
        : '';
      if (!description) {
        warnings.push(
          `evals: ${rel}: missing description; state the one production behavior this eval enforces and how must criteria prove it`,
        );
      } else if (description.length < EVAL_DESCRIPTION_MIN_CHARS) {
        warnings.push(
          `evals: ${rel}: description is very short; state the behavior under test and how must proves it`,
        );
      }
