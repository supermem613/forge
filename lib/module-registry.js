...
async function loadModule(moduleSpec, registry) {
  ...
  try {
    ...
    if (err.code === 'ENOENT') {
      // A listed sibling may be absent on this machine. Skip it so other
      // modules still load. Missing is not a parse error in the present module.
      process.stderr.write(`forge: skip missing module ${modulePath}\n`);
      return;
    }
    throw err;
  }
...
export async function findStrayRunbookFiles(registry) {
  const strays = [];
  for (const root of registry.runbookRoots()) {
    let entries;
    try {
    ...
    for (const entry of entries) {
      // listRunbooks skips non-directories, so a loose file here is invisible
      // to forge while still reading as authoritative to the next author.
      // README.md is the catalog and is expected.
      if (!entry.isDirectory() && entry.name !== 'README.md') {
        strays.push({ name: entry.name, root, path: path.join(root, entry.name) });
      }
    }
  }
  return strays;
