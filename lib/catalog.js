...
// The header and divider may use CRLF. Matching only `\n` here silently broke
// catalog registration on every Windows checkout, and the caller treats that
// failure as non-fatal, so runbooks went unlisted with no visible error.
const CATALOG_HEADER_RE = /(^##[ \t]+Catalog[ \t]*\r?\n(?:[ \t]*\r?\n)*)\|[ \t]*Runbook[ \t]*\|[ \t]*What it evaluates[ \t]*\|[ \t]*\r?\n\|[-\t |]+\|[ \t]*\r?\n/m;
