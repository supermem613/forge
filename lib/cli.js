...
import { scaffoldRunbook, resolveRunbooksDir } from './cli-new-runbook.js';
...
import { packAnalysis, verifyAnalysis } from './cli-pack-analysis.js';
import { updateForge } from './cli-update.js';
import { checkRunArtifact } from './run-artifact-check.js';
import { readRunAdoption, checkAdoptionGate } from './adoption.js';
import { loadForgeRegistry, listRunbooks as listRegisteredRunbooks, findRunbook, findStrayRunbookFiles } from './module-registry.js';
...
// Global presentational flag. PowerShell treats any native-command stderr write
// as a NativeCommandError and surfaces a red error even on a clean exit 0. The
// JSON success envelope already carries everything a caller needs on stdout, so
// `--quiet` suppresses forge's own stderr chatter (progress logs and the final
// error echo). It does not change exit codes or stdout. Parsed from raw argv so
// it is available before per-command flag parsing.
...
async function runStep({ registry, runbook, runbookDir, experiment, step, args, capture = false }) {
  ...
  try {
    return await new Promise((resolve, reject) => {
      // When capture is set, the child's stdout (its own JSON envelope) is
      // absorbed rather than inherited so a wrapper command like `grade` can
      // emit a single envelope of its own. stderr still inherits so progress
      // remains visible.
    ...
    if (step === 'run') {
      ...
      if (!cleanExit) {
        await postAbortCleanup({ registry, runbook, runbookDir, experiment, reason: cleanupRequested ? 'sigint' : 'child-error' }).catch((e) => {
          elog(`forge: post-abort cleanup error: ${e.message}\n`);
        });
      }
    }
  }
...
async function main() {
  ...
  // `forge <subcommand> --help` prints that command's schema slice rather than
  // erroring. This is the per-command discovery surface: an agent can ask any
  // command how to call it without parsing the full `forge schema` catalog.
  if (cmd && cmd !== 'schema' && (rest.includes('--help') || rest.includes('-h'))) {
  ...
  if (cmd === 'list') {
    ...
    // Loose files in a runbook root are skipped by listRunbooks, so surface
    // them here; silence is what let hand-written non-runbooks accumulate.
    ...
    emit('list', {
      ...
      ...(strays.length ? {
        warnings: strays.map(s => `ignored non-runbook file in ${s.root}: ${s.name} — a runbook must be a directory; run \`forge new-runbook <id>\` or move prose out of the runbook root`),
      } : {}),
    });
    return;
  }
  ...
  if (cmd === 'doctor') {
    ...
    if (dflags.fix) {
      ...
      if (swept.length) {
        elog(`forge doctor: swept ${swept.length} stale resource lock(s)\n`);
      }
    }
    if (dflags.wait) {
  ...
  if (cmd === 'adoption') {
    ...
    const expName = aargs[0];
    if (!expName) {
      throw new ForgeError('adoption: <experiment> required', { code: 'USAGE', hint: 'forge adoption <experiment> [--variant mark-N] [--expect-verb files.select] [--min-rate 0.5] [--max-hand-reduce 0.3] [--forbid-dead] [--known-verbs a,b]' });
    }
    let runDir = aflags.run && aflags.run !== true ? path.resolve(String(aflags.run)) : null;
    if (!runDir) {
      const runs = await listRuns(expName);
      if (!runs.length) {
        throw new ForgeError(`adoption: no runs for experiment '${expName}'`, { code: 'NOT_FOUND', hint: 'Run `forge run <experiment> <variant>` first, or pass --run <dir>.' });
      }
      const wantVariant = aflags.variant && aflags.variant !== true ? String(aflags.variant) : null;
      const pool = wantVariant ? runs.filter(r => r.variant === wantVariant) : runs;
      if (!pool.length) {
        throw new ForgeError(`adoption: no runs for variant '${wantVariant}' in '${expName}'`, { code: 'NOT_FOUND', hint: 'Run `forge runs <experiment>` to list runs.' });
      }
      runDir = pool[pool.length - 1].dir; // latest by timestamp
    }
    const knownVerbs = aflags['known-verbs'] && aflags['known-verbs'] !== true
      ? String(aflags['known-verbs']).split(',').map(s => s.trim()).filter(Boolean) : [];
    const adoption = await readRunAdoption(runDir, { knownVerbs });
    // A bad --run path or a run that never wrote turn1/ artifacts must NOT
    // false-green the pre-build probe. Treat no-data as a hard failure.
    if (adoption.source && adoption.source.readError) {
      throw new ForgeError(`adoption: no turn1 artifacts under '${runDir}'`, { code: 'NO_ARTIFACTS', hint: 'Point --run at a run dir that contains turn1/<eval>-sampleN.json, or run the experiment first.' });
    }
    if (adoption.n === 0) {
      throw new ForgeError(`adoption: 0 turn1 samples under '${runDir}'`, { code: 'NO_SAMPLES', hint: 'The run produced no turn1 samples to measure. Re-run the experiment before probing adoption.' });
    }
    const gate = {};
    if (aflags['expect-verb'] && aflags['expect-verb'] !== true) {
      gate.expectVerb = String(aflags['expect-verb']);
    }
    if (aflags['min-rate'] && aflags['min-rate'] !== true) {
      gate.minCalledRate = Number(aflags['min-rate']);
    }
    if (aflags['max-hand-reduce'] && aflags['max-hand-reduce'] !== true) {
      gate.maxHandReduceRate = Number(aflags['max-hand-reduce']);
    }
    if (aflags['forbid-dead']) {
      gate.forbidDeadVerbs = true;
    }
    const hasGate = Object.keys(gate).length > 0;
    const gateResult = hasGate ? checkAdoptionGate(adoption, gate) : { pass: true, failures: [] };
    const ok = gateResult.pass;
    envelope('adoption', {
      experiment: expName,
      runDir,
      n: adoption.n,
      perVerb: adoption.perVerb,
      handReduce: adoption.handReduce,
      deadVerbs: adoption.deadVerbs,
      source: adoption.source || null,
      bySample: adoption.bySample,
      gate: hasGate ? { ...gate, ...gateResult } : null,
    }, ok);
    process.exit(ok ? 0 : 1);
  }
  ...
  if (cmd === 'new-runbook') {
    ...
    const id = nargs[0];
    if (!id) {
      throw new ForgeError('new-runbook: <id> required', { code: 'USAGE', hint: 'forge new-runbook <id> [--module <name>] [--runbooks-dir <path>] [--description "..."]' });
    }
    const result = await scaffoldRunbook({
      runbookId: id,
      runbooksDir: await resolveRunbooksDir({
        repoRoot: REPO_ROOT,
        moduleName: nflags.module && nflags.module !== true ? String(nflags.module) : null,
        runbooksDir: nflags['runbooks-dir'] && nflags['runbooks-dir'] !== true ? String(nflags['runbooks-dir']) : null,
      }),
  ...
  if (cmd === 'recommend') {
    const { buildRecommendation } = await import('./cli-recommend.js');
    const pair = (typeof flags.pair === 'string') ? flags.pair : 'latest';
    const out = await buildRecommendation({ experiment, pair, repoRoot: REPO_ROOT });
    emit('recommend', { markdown: out.markdown, variant: out.variantName, suggestionCount: out.suggestionCount });
    return;
  }

  if (cmd === 'grade') {
    const { json: gExpJson } = await readExperiment(experiment);
    const gRunbook = gExpJson.runbook;
    const gRunbookInfo = await findRunbook(registry, gRunbook);
    if (!gRunbookInfo) {
      throw new ForgeError(`no such runbook: ${gRunbook}`, { code: 'NOT_FOUND', hint: 'Run `forge list` to see available runbooks.' });
    }
    const pair = (typeof flags.pair === 'string') ? flags.pair : null;
    const prev = (typeof flags.prev === 'string') ? flags.prev : null;
    const finalize = !!flags.finalize;
    const boundRunStep = ({ step, args: stepArgs, capture }) => runStep({
      registry, runbook: gRunbook, runbookDir: gRunbookInfo.dir, experiment, step,
      args: [...stepArgs, '--forge-root', REPO_ROOT], capture,
    });
    const { runGrade } = await import('./cli-grade.js');
    const { resolveEvalKind } = await import('./judge-orchestrator.js');
    const gManifest = JSON.parse(await fs.readFile(path.join(gRunbookInfo.dir, 'manifest.json'), 'utf8'));
    const evalKind = resolveEvalKind(gManifest);
    const result = await runGrade({ experiment, pair, prev, finalize, runStep: boundRunStep, evalKind });
    emit('grade', result);
    return;
  }
  ...
  if (cmd === 'pack-analysis') {
    const toPath = typeof flags.to === 'string' ? flags.to : null;
    const claimClass = typeof flags.depth === 'string' ? flags.depth
      : (typeof flags['claim-class'] === 'string' ? flags['claim-class'] : 'audit');
    const result = await packAnalysis({
      repoRoot: REPO_ROOT,
      experimentsDir: EXPERIMENTS_DIR,
      registry,
      experiment,
      toPath,
      claimClass,
      includeCaptures: !!flags['include-captures'],
      dryRun: !!flags['dry-run'],
      forgeVersion: await packageVersion(),
    });
    emit('pack-analysis', result);
    return;
  }

  if (cmd === 'verify-analysis') {
    // Positional may be the zip path when no experiment is required.
    const zipPath = (typeof flags.zip === 'string' && flags.zip)
      || experiment
      || (typeof args[1] === 'string' ? args[1] : null);
    const result = await verifyAnalysis({ zipPath });
    emit('verify-analysis', result);
    return;
  }
