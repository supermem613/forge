...
import { archiveCaptures as archiveCaptureWindow } from './capture-archive.js';
...
export async function openBundle({ experiment, variant, runsDir, ts: timestamp, loadedSkills = () => [] }) {
  ...
  return buildBundleApi({ dir, stamp, manifest, manifestPath, transcript, transcriptPath, loadedSkills });
}

// Reuse mode for `forge resample`. Resolves the newest <ts> bundle under
// `runsDir` and returns the SAME writer API pointed at that existing dir. The
// loaded manifest and transcript are carried forward unchanged: startedAt is
// preserved and no prior artifact is reset, so re-running one sample repairs
// that turn in place without minting a new timestamped dir or breaking the
// paired control/variant comparison, which score.js keys on the run dir.
// writeTurn overwrites the targeted turn file and de-dupes its transcript
// entry; new files stay append-only.
export async function reopenBundle({ runsDir, loadedSkills = () => [] }) {
  if (!runsDir) {
    throw new Error('reopenBundle: runsDir is required (caller decides where bundles live)');
  }
  const dir = await latest({ runsDir });
  if (!dir) {
    throw new Error(`reopenBundle: no existing run bundle under ${runsDir} to reuse`);
  }
  const manifestPath = path.join(dir, 'manifest.json');
  const manifest = await readBundleManifest(dir);
  const stamp = manifest.ts || path.basename(dir);
...
function buildBundleApi({ dir, stamp, manifest, manifestPath, transcript, transcriptPath, loadedSkills }) {
  ...
  // Archive a producer's raw capture window into this bundle so the run stays
  // self-contained and mineable. `sourceDir` and the window are producer
  // knowledge the caller supplies. The summary is recorded in the manifest so
  // downstream tools can find the captures without re-globbing a shared dir.
  async function archiveCaptures({ sourceDir, since, until, slackMs = 0 }) {
    const destDir = path.join(dir, 'captures');
    const summary = await archiveCaptureWindow({ sourceDir, destDir, since, until, slackMs });
    manifest.captures = summary;
    await fs.writeFile(manifestPath, JSON.stringify(manifest, null, 2) + '\n');
    return summary;
  }
  ...
  return { dir, ts: stamp, writeRaw, writeMd, writeTurn, pairWith, archiveCaptures, finalize };
}
