...
function parseSpec(spec) {
  const m = /^([^:]+)(?::(.+))?$/.exec(spec);
  if (!m) {
    throw new ForgeError(`compare: bad variant spec "${spec}"`, { code: 'USAGE', hint: 'Variant spec must be control, mark-N, or mark-N:<timestamp>.' });
  }
...
async function latestRunDir({ expDir, variant }) {
  ...
  try {
    ...
    if (e.code === 'ENOENT') {
      throw new ForgeError(`compare: no runs for variant "${variant}"`, { code: 'NOT_FOUND', hint: `Run \`forge run <exp> ${variant}\` to produce a run first.` });
    }
    throw e;
  }
...
async function loadReport({ expDir, spec }) {
  ...
  try {
    ...
    if (e.code === 'ENOENT') {
      throw new ForgeError(`compare: no REPORT.json in ${runDir} (run \`forge report\` first)`, { code: 'NOT_FOUND', hint: 'Run `forge report <exp>` to generate REPORT.json first.' });
    }
    throw e;
  }
