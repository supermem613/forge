...
export async function buildPresentation({ experiment, pair = 'latest', repoRoot }) {
  ...
  if (!variantRun) {
    throw new Error(`present: no variant run for experiment '${experiment}' (pair=${pair}) — control-only baselines have nothing to compare`);
  }
  let report;
  try {
    ...
    if (e.code === 'ENOENT') {
      throw new Error(`present: REPORT.json missing under ${variantRun}. Run \`forge report ${experiment}\` first.`);
    }
    throw e;
  }
