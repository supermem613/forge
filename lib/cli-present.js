...
// headline · learnings · per-sample heatmap · top criterion contrasts. This is
// the data-only analysis surface. Prescriptive next-step advice lives in
// `forge recommend`, never here. The orchestrator agent is expected to pipe
// this straight to the user without re-summarizing.
...
export async function buildPresentation({ experiment, pair = 'latest', repoRoot }) {
  ...
  // ─── Learnings ───────────────────────────────────────────────────────────
  // ─── Learnings (data-only observations) ──────────────────────────────────
  // Breakthroughs and pitfalls are observations grounded in the run. Prescriptive
  // suggestions are intentionally excluded here and surface via `forge recommend`.
  const ins = report.insights || {};
  if ((ins.breakthroughs || []).length || (ins.pitfalls || []).length) {
  ...
  // ─── Pointer to recommendations (kept separate from the data) ─────────────
  lines.push('## Recommendations');
  lines.push('');
  lines.push(`Analysis above is data-only. Run \`forge recommend ${experiment}\` for prescriptive next-step guidance.`);
  lines.push('');
