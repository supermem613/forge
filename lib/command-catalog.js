export const CORE_COMMANDS = Object.freeze([
  {
    ...
    output: { documented: true, schema: 'RunbookListText' },
    examples: ['forge list'],
  },
  {
    path: ['experiments'],
    ...
    effect: 'read',
    input: { positionals: [], flags: [] },
    output: { documented: true, schema: 'ExperimentListText' },
  },
  {
    path: ['schema'],
  ...
  },
  {
    ...
    effect: 'mutate-local',
    input: { positionals: [], flags: [] },
    output: { documented: true, schema: 'ConfigPathText' },
  },
  {
  ...
  {
    path: ['judge'],
    usage: 'judge <exp> [--pair latest|<spec>] [--mode agent|collect]',
    summary: 'Run agent-as-judge or collect judge verdicts.',
    effect: 'mutate-local',
    input: {
      positionals: [{ name: 'exp', required: true }],
      flags: [{ name: 'pair', type: 'string' }, { name: 'mode', type: 'string' }],
    },
    output: { documented: false },
    examples: ['forge judge prompt-routing --pair latest'],
  },
  {
    path: ['report'],
  ...
  },
  {
    ...
    summary: 'Emit a paste-ready summary.',
    effect: 'read',
    input: {
      positionals: [{ name: 'exp', required: true }],
      flags: [{ name: 'pair', type: 'string' }],
    },
    output: { documented: true, schema: 'PresentationMarkdown' },
  },
  {
    path: ['teardown'],
    usage: 'teardown <exp>',
  ...
  {
    path: ['artifact-check'],
    usage: 'artifact-check <runDir> [--json]',
  ...
  {
    path: ['compare'],
    usage: 'compare <exp> <variantA> <variantB> [--json]',
  ...
  {
    path: ['resample'],
    usage: 'resample <exp> <variant> --eval <id> --sample <N>',
    summary: 'Re-run one sample in the latest run for a variant.',
    effect: 'mutate-local',
    input: {
      positionals: [{ name: 'exp', required: true }, { name: 'variant', required: true }],
  ...
  {
    path: ['doctor'],
    usage: 'doctor [--json] [--fix] [--wait] [--timeout MS] [--interval MS]',
...
export function moduleCommandsForSchema(registry) {
  return [...registry.commands().keys()].sort().map((name) => ({
    path: [name],
    summary: 'Module-registered command.',
    effect: 'mutate-local',
    input: { positionals: [], flags: [] },
    output: { documented: false },
    moduleCommand: true,
...
export function buildSchema({ version, registry, commandPrefix = [], summary = false }) {
  ...
  return {
    ...
    cliVersion: version,
    envelope: {
      stdout: 'JSON only for schema and --json command modes. Legacy human commands write human text to stdout.',
      stderr: 'Progress, diagnostics, and human-readable errors.',
