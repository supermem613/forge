export const CORE_COMMANDS = Object.freeze([
  ...
  {
    path: ['run'],
    usage: 'run <exp> <control|mark-N> [--samples N] [--evalIds a,b] [--append-control] [--asymmetric] [--capture] [--<runbook-flag> <value>]',
    summary: 'Run samples for a control or variant. Unknown flags (e.g. a runbook-required --spfx-dev-server) are forwarded verbatim to the runbook run step.',
    effect: 'mutate-local',
    input: {
      positionals: [{ name: 'exp', required: true }, { name: 'variant', required: true }],
      flags: [
        { name: 'samples', type: 'number' },
        { name: 'evalIds', type: 'string' },
        { name: 'append-control', type: 'boolean' },
        { name: 'asymmetric', type: 'boolean' },
        { name: 'capture', type: 'boolean' },
        { name: 'forwarded', type: 'string', note: 'Any other --flag is forwarded unchanged to the runbook run step. Runbooks may require their own, e.g. spark-scenario-efficiency requires --spfx-dev-server <url>.' },
      ],
    },
    output: { documented: false },
  ...
  {
    path: ['score'],
    usage: 'score <exp> [--pair latest|<spec>] [--prev mark-N]',
    summary: 'Run deterministic scoring of a paired run.',
    effect: 'mutate-local',
    input: {
      positionals: [{ name: 'exp', required: true }],
      flags: [{ name: 'pair', type: 'string' }, { name: 'prev', type: 'string' }],
    },
    output: { documented: false },
    examples: ['forge score prompt-routing --pair latest', 'forge score prompt-routing --prev mark-1'],
  },
  {
    path: ['judge'],
  ...
  {
    path: ['grade'],
    usage: 'grade <exp> [--pair latest|<spec>] [--finalize] [--prev mark-N]',
    summary: 'Two-phase judging convenience. Default (evalKind=judge) prepares prompts and prints the gpt-5.5 dispatch contract; --finalize validates, collects, and scores. evalKind=oracle skips dispatch (judgeDispatches: 0) and --finalize scores from score.json.',
    effect: 'mutate-local',
    input: {
      positionals: [{ name: 'exp', required: true }],
      flags: [
        { name: 'pair', type: 'string' },
        { name: 'finalize', type: 'boolean' },
        { name: 'prev', type: 'string' },
      ],
    },
    output: { documented: true, schema: 'GradeResult' },
    examples: [
      'forge grade prompt-routing',
      'forge grade prompt-routing --finalize',
    ],
  },
  {
    path: ['report'],
    usage: 'report <exp> [--pair latest|<spec>] [--prev mark-N]',
    ...
    examples: ['forge report prompt-routing --pair latest', 'forge report prompt-routing --prev mark-1'],
  },
  {
    path: ['present'],
    usage: 'present <exp> [--pair latest|<spec>]',
    summary: 'Emit a paste-ready, data-only summary as data.markdown. No prescriptions; use `recommend` for those.',
    effect: 'read',
    input: {
      positionals: [{ name: 'exp', required: true }],
  ...
  {
    path: ['recommend'],
    usage: 'recommend <exp> [--pair latest|<spec>]',
    summary: 'Emit prescriptive next-step guidance as data.markdown. The deliberate counterpart to the data-only report/present surfaces.',
    effect: 'read',
    input: {
      positionals: [{ name: 'exp', required: true }],
      flags: [{ name: 'pair', type: 'string' }],
    },
    output: { documented: true, schema: 'RecommendationResult' },
    examples: ['forge recommend prompt-routing', 'forge recommend prompt-routing --pair latest'],
  },
  {
    path: ['teardown'],
    usage: 'teardown <exp>',
  ...
  {
    path: ['pack-analysis'],
    usage: 'pack-analysis <exp> --to <path.tar.zst> [--depth audit] [--dry-run]',
    summary: 'Non-destructive curated audit pack of an experiment plus runbook snapshot.',
    effect: 'read',
    input: {
      positionals: [{ name: 'exp', required: true }],
      flags: [
        ...
        { name: 'claim-class', type: 'string' },
        { name: 'include-captures', type: 'boolean' },
        { name: 'dry-run', type: 'boolean' },
      ],
    },
    output: { documented: true, schema: 'PackAnalysisResult' },
    examples: ['forge pack-analysis prompt-routing --to ./prompt-routing-audit.tar.zst'],
  },
  {
    path: ['verify-analysis'],
    usage: 'verify-analysis <path.tar.zst>',
    summary: 'Verify an analysis pack zip against its ANALYSIS.json inventory.',
    effect: 'read',
    input: {
      positionals: [{ name: 'zip', required: true }],
      flags: [],
    },
    output: { documented: true, schema: 'VerifyAnalysisResult' },
    examples: ['forge verify-analysis ./prompt-routing-audit.tar.zst'],
  },
  {
    path: ['runs'],
    usage: 'runs <exp>',
  ...
  },
  {
    path: ['update'],
    usage: 'update',
    summary: 'Self-update with pull (auto-detects soda vs git), npm install, build, and link.',
    effect: 'mutate-local',
    input: { positionals: [], flags: [] },
...
export function buildSchema({ version, registry, commandPrefix = [], summary = false }) {
  ...
  return {
    ...
    globalFlags: [
      { name: 'help', aliases: ['h'], type: 'boolean', note: 'On any subcommand, prints that command\'s schema slice instead of running it.' },
      { name: 'version', type: 'boolean' },
      { name: 'quiet', type: 'boolean', note: 'Suppress forge\'s own stderr chatter and final error echo. Stdout JSON and exit codes are unchanged. Use it to avoid PowerShell NativeCommandError noise on clean runs.' },
    ],
    commands,
    errorCodes: [
