...
function buildArtifact(sample, turn1, turn2, loadedSkills) {
  ...
  const td = turn1?.response?.toolDetails || [];
  if (td.length === 0) {
...
export async function runJudge({ argv, runbookDir, repoRoot, log, loadedSkills = () => [] } = {}) {
  ...
  if (mode === 'agent') {
    onLog(`next: agent reads each judge-prompts/*.md and writes JSON to ${refitStale ? 'judge-verdicts-refit/<criteriaHash>/' : 'judge-verdicts/'}, then run --mode collect${refitStale ? ' --refit-stale' : ''}`);
    onLog(`policy: dispatch a sub-agent with model='gpt-5.4' to judge — see .claude/skills/experiment/SKILL.md "Judge model policy".`);
  } else {
    const allMissing = summary.reduce((n, r) => n + r.missing.length, 0);
    if (allMissing > 0) {
      onLog(`WARNING: ${allMissing} verdict file(s) missing. Score will treat missing samples as auto-fail.`);
    } else {
      onLog(`all verdicts present. Run \`forge score ${experiment}\` next.`);
    }
  }
