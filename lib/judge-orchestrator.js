...
export async function runJudge({ argv, runbookDir, repoRoot, log, loadedSkills = () => [] } = {}) {
  ...
  if (mode === 'agent') {
    ...
    if (allMissing > 0 || allInvalid > 0) {
      // Fail loud: a cheerful "all verdicts present" while files are missing or
      // wrong-shape sends score into a null/zero-tier result that looks like a
      // pass. Surface the blocked count and refuse the green light.
