...
export function resolveEvalKind(manifest) {
  const kind = manifest?.evalKind;
  if (kind == null || kind === '') {
    return 'judge';
  }
  if (kind === 'oracle' || kind === 'judge' || kind === 'hybrid') {
    return kind;
  }
  throw new Error(`evalKind must be oracle|judge|hybrid (got '${kind}')`);
}
...
function renderReasoning(lines, response) {
  const segments = Array.isArray(response?.reasoning)
    ? response.reasoning.filter(s => typeof s === 'string' && s.trim().length > 0)
    : [];
  if (segments.length === 0) {
    return;
  }
  // The model's own ChainOfThought for this solve. INVARIANT the answer text
  // never carries reasoning narration, so this is the only place the judge can
  // weigh whether the approach, not just the final answer, was sound.
  lines.push(`Model reasoning (chain of thought, ${segments.length} segment(s)):`);
  for (let i = 0; i < segments.length; i++) {
    lines.push(`  ${i + 1}. ${segments[i]}`);
  }
}
...
function buildArtifact(sample, turn1, turn2, loadedSkills) {
  ...
  if (!turn2) {
    ...
    renderReasoning(lines, turn1?.response);
  ...
  renderReasoning(lines, turn2?.response);
...
async function processVariant({ runDir, variantLabel, evals, mode, evalKind, refitStale, log, loadedSkills }) {
...
export async function runJudge({ argv, runbookDir, repoRoot, log, loadedSkills = () => [] } = {}) {
  ...
  // Oracle runbooks refuse: there is no gpt-5.5 judge contract.
  const { evals, manifest } = await loadEvals(runbookDir);
  const evalKind = resolveEvalKind(manifest);

  if (dispatchPrompt) {
    if (evalKind === 'oracle') {
      const err = new Error('evalKind=oracle: gpt-5.5 dispatch is not allowed');
      err.code = 'JUDGE_ORACLE_NO_DISPATCH';
      throw err;
    }
    const prompt = buildDispatchPrompt({ controlRun, variantRun, variantName, refitStale });
    process.stdout.write(prompt + '\n');
    return { controlRun, variantRun, variantName, dispatchPrompt: true, evalKind, judgeDispatches: 1 };
  }
  ...
  for (const a of variants) {
    const r = await processVariant({ runDir: a.runDir, variantLabel: a.label, evals, mode, evalKind, refitStale, log: onLog, loadedSkills });
    summary.push(r);
    if (mode === 'agent') {
  ...
  const judgeDispatches = mode === 'agent' && evalKind === 'oracle' ? 0 : (mode === 'agent' ? 1 : 0);
  for (const r of summary) {
    const status = {
      mode, refit: !!refitStale, evalKind,
  ...
  if (mode === 'agent' && evalKind !== 'oracle') {
    ...
    onLog(`oracle: skipped gpt-5.5 dispatch (judgeDispatches=${judgeDispatches}). Score from deterministic expectedPass / score.json.`);
  } else {
  ...
  return { summary, controlRun, variantRun, variantName, refitStale, evalKind, judgeDispatches };
}
