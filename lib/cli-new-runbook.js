...
// Resolve where a new runbook should be scaffolded. A runbook belongs next to
// the code that runs it, so a module author must be able to target their own
// module's runbook root; without this they had no supported path and resorted
// to hand-writing files that forge could never list or validate.
export async function resolveRunbooksDir({ repoRoot, moduleName, runbooksDir, configPath } = {}) {
  if (runbooksDir) {
    return path.resolve(runbooksDir);
  }
  if (!repoRoot) {
    throw new Error('resolveRunbooksDir: repoRoot required');
  }
  if (!moduleName) {
    return path.join(repoRoot, 'runbooks');
  }

  const cfgPath = configPath || path.join(repoRoot, 'forge.config.json');
  let config;
  try {
    config = JSON.parse(await fs.readFile(cfgPath, 'utf8'));
  } catch {
    throw new Error(`resolveRunbooksDir: cannot read ${cfgPath} to resolve module "${moduleName}"`);
  }
  const modules = Array.isArray(config.modules) ? config.modules : [];
  const match = modules.find(m => m && m.name === moduleName);
  if (!match) {
    const known = modules.map(m => m && m.name).filter(Boolean).join(', ') || '(none configured)';
    throw new Error(`resolveRunbooksDir: no such module "${moduleName}"; known modules: ${known}`);
  }
  return path.join(path.resolve(repoRoot, match.path || match), 'runbooks');
}
...
export async function scaffoldRunbook({ runbookId, runbooksDir, description } = {}) {
  ...
  const exampleEval = {
    id: 'example',
    description:
      'Enforces one production behavior for this runbook. Proof: must criteria observe a concrete outcome. Replace this scaffold.',
