...
// ANALYSIS.json, zips the staging tree, and self-verifies before success.
...
import { zipDirContents, readZipEntries } from './zip.js';
...
export async function packAnalysis({
  ...
  if (!experiment) {
    throw new ForgeError('pack-analysis: <experiment> required', {
      code: 'USAGE',
      hint: 'forge pack-analysis <experiment> --to <out.zip>',
    });
  }
  ...
  if (!toPath) {
    throw new ForgeError('pack-analysis: --to <path.zip> required', {
      code: 'USAGE',
      hint: 'forge pack-analysis <experiment> --to <out.zip>',
    });
  }
  ...
  if (!outAbs.toLowerCase().endsWith('.zip')) {
    throw new ForgeError('pack-analysis: --to must end with .zip', {
      code: 'USAGE',
      hint: 'Pass an output path like ./exp-audit.zip',
    });
  }
  if (await isInside(expDir, outAbs)) {
    throw new ForgeError('pack-analysis: --to is inside the source experiment dir', {
      code: 'VALIDATION_FAILED',
      hint: 'Write the zip outside the experiment directory.',
    });
  }
  ...
  for (const v of variants) {
    ...
    for (const r of runs) {
      if (!r.isDirectory()) {
        // Already-zipped runs from archive are not audit sources in-place.
        if (r.isFile() && r.name.endsWith('.zip')) {
          const st = await fs.stat(path.join(runsDir, r.name));
          await noteExclude(
            `experiment/variants/${vName}/runs/${r.name}`,
            'zipped-run-not-unpacked',
            st.size,
          );
        }
        continue;
  ...
  if (await pathExists(outAbs)) {
    throw new ForgeError(`pack-analysis: destination already exists: ${outAbs}`, {
      code: 'CONFLICT',
      hint: 'Choose a new --to path or remove the existing zip.',
    });
  }
  ...
  try {
    ...
    const { files: zipCount } = await zipDirContents(stageDir, outAbs);
    const zipStat = await fs.stat(outAbs);
    analysis.totals.zipBytes = zipStat.size;
    // Rewrite ANALYSIS.json inside a temp restage is expensive; instead embed
    // zipBytes only in the command envelope. The on-disk ANALYSIS.json keeps
    // zipBytes null and verify does not require it.

    // Re-open zip and confirm ANALYSIS.json + entry count.
    const zipBuf = await fs.readFile(outAbs);
    const entries = readZipEntries(zipBuf);
    if (!entries.has('ANALYSIS.json')) {
      await fs.rm(outAbs, { force: true });
      throw new ForgeError('pack-analysis: zip missing ANALYSIS.json after write', {
        code: 'VALIDATION_FAILED',
        hint: 'Retry pack-analysis; if it persists, file a forge bug.',
      });
    }
    if (entries.size !== zipCount) {
      await fs.rm(outAbs, { force: true });
      throw new ForgeError(
        `pack-analysis: zip entry count ${entries.size} != writer count ${zipCount}`,
...
export async function verifyAnalysis({ zipPath } = {}) {
  if (!zipPath) {
    throw new ForgeError('verify-analysis: <zip> required', {
      code: 'USAGE',
      hint: 'forge verify-analysis <path.zip>',
    });
  }
  ...
  try {
    ...
    throw new ForgeError(`verify-analysis: invalid zip: ${e.message}`, {
      code: 'VALIDATION_FAILED',
      hint: 'Repack with `forge pack-analysis`.',
    });
  }
  ...
  if (!analysisBuf) {
    throw new ForgeError('verify-analysis: ANALYSIS.json missing from zip', {
      code: 'VALIDATION_FAILED',
      hint: 'Repack with `forge pack-analysis`.',
    });
  }
  ...
  for (const entry of analysis.included) {
    ...
    if (!data) {
      errors.push(`included path missing in zip: ${entry.path}`);
      continue;
    }
