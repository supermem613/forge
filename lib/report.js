...
import { resolvePair, resolvePrevVariant } from './run-pair.js';
...
export async function runReport({ argv, repoRoot, log } = {}) {
  ...
  // simply drops the trend section.
  const { prevVariantRun, prevVariantName } = await resolvePrevVariant(expDir, variantName);
  let prev = null;
  if (prevVariantRun) {
    try {
      prev = JSON.parse(await fs.readFile(path.join(prevVariantRun, 'score.json'), 'utf8'));
    } catch {
      prev = null;
    }
  }
