...
// Render an efficiency change as an unambiguous direction word rather than a
// bare signed "% saved". A reader skimming the table must never have to recall
// the delta = control − variant sign convention. pctSaved > 0 means the variant
// spent less (lower latency or fewer tokens); pctSaved < 0 means it spent more.
// Time-axis metrics read as faster/slower, count metrics as fewer/more.
function fmtChange(key, pctSaved) {
  ...
  const better = pctSaved > 0;
  const word = isTime ? (better ? 'faster' : 'slower') : (better ? 'fewer' : 'more');
  return `${Math.abs(pctSaved)}% ${word}`;
}
