...
import {
  fmtChange as metricsFmtChange,
  humanizeMetric as metricsHumanize,
...
// Direction-aware change wording from the metrics catalog (lib/metrics.js).
// pctSaved > 0 still means the variant value is lower than control. Catalog
// direction decides faster/slower, fewer/more, more/less, or lower/higher.
function fmtChange(key, pctSaved) {
  return metricsFmtChange(key, pctSaved);
}

function humanizeMetric(key) {
  return metricsHumanize(key);
}
...
function buildHeatmap(variantScore) {
  ...
  for (const ev of variantScore.evals) {
    const sampleMatrix = Array.isArray(ev.sampleMatrix) ? ev.sampleMatrix : [];
    for (const tier of TIERS) {
      const cells = [];
      let totalPass = 0, totalAll = 0;
      for (let r = 1; r <= samples; r++) {
        const row = sampleMatrix.find(m => m.sample === r);
...
export async function runReport({ argv, repoRoot, log } = {}) {
  ...
  // Metrics provenance from pair.json. Prefer explicit control/variant blocks
  // when present; otherwise accept a shared declareMetrics() object. Legacy
  // pair.json without metrics stays comparable.
  const metricsBlock = pairData?.metrics || null;
  let metricsComparable = true;
  let metricsCompareReason = null;
  if (metricsBlock?.control && metricsBlock?.variant) {
    const cmp = compareScoredProfiles(metricsBlock.control, metricsBlock.variant);
    metricsComparable = cmp.comparable;
    metricsCompareReason = cmp.reason;
  } else if (metricsBlock && metricsBlock.comparable === false) {
    metricsComparable = false;
    metricsCompareReason = metricsBlock.reason || 'pair marked metrics incomparable';
  }
  ...
  if (pairData?.efficiency) {
    ...
    lines.push('');
    if (metricsBlock) {
      const shared = metricsBlock.scored
        ? metricsBlock
        : (metricsBlock.variant || metricsBlock.control || null);
      if (shared?.scored) {
        lines.push(`Metrics profile: scored \`${shared.scored}\` · captured \`${shared.captured || shared.scored}\` · catalog v${shared.catalogVersion}${shared.kashVersion ? ` · kash ${shared.kashVersion}` : ''}.`);
        lines.push('');
      }
      if (!metricsComparable) {
        lines.push(`**Incomparable metrics:** ${metricsCompareReason || 'scored profiles or catalog versions differ'}. Efficiency deltas below are not an honest pair.`);
        lines.push('');
      }
    }
    if (effThreeWay) {
      lines.push(`Every model-context metric across all three points. **Δ vs prev** is the raw current-minus-previous difference; **change vs prev** states the direction (faster/slower or fewer/more) of current ${variantName} relative to previous ${prevTrend.variantName}.`);
      lines.push('');
      lines.push(`| metric | control | ${prevTrend.variantName} | ${variantName} | Δ vs prev | change vs prev |`);
      lines.push('|--------|--------:|--------:|--------:|---------:|---------------:|');
      for (const [key, m] of Object.entries(effThreeWay)) {
        lines.push(`| ${humanizeMetric(key)} | ${fmtNum(m.control)} | ${fmtNum(m.previous)} | ${fmtNum(m.variant)} | ${fmtNum(m.deltaVsPrev)} | ${fmtChange(key, m.pctSavedVsPrev)} |`);
      }
    } else {
      lines.push(`Every model-context metric, variant vs control. **change** states the direction (faster/slower or fewer/more) the variant moved relative to control.`);
      lines.push('');
      lines.push(`| metric | control | ${variantName} | Δ | change |`);
      lines.push('|--------|--------:|--------:|--:|--------:|');
      for (const [key, m] of Object.entries(pairData.efficiency)) {
        lines.push(`| ${humanizeMetric(key)} | ${fmtNum(m.control)} | ${fmtNum(m.variant)} | ${fmtNum(m.delta)} | ${fmtChange(key, m.pctSaved)} |`);
      }
    }
    lines.push('');
  ...
  // ─── Insights ───────────────────────────────────────────────────────────
  // Data-only: observations grounded in the run (breakthroughs + pitfalls).
  // Prescriptive next-variant suggestions are deliberately NOT rendered here.
  // They live in REPORT.json (insights.suggestions) and surface only through
  // the separate `forge recommend` step, keeping analysis and advice apart.
  ...
  if (insights.suggestions.length) {
    onLog(`recommendations: ${insights.suggestions.length} (run \`forge recommend ${experiment}\` to view)`);
  }
