...
function parseArgs(argv) {
  const out = { experiment: null, pair: 'latest' };
...
export async function runReport({ argv, repoRoot, log } = {}) {
  ...
  const { experiment, pair } = parseArgs(argv || []);
  if (!experiment) {
  ...
  const reportJson = {
    ...
    headline: { overall: overallDelta, tiers: tierDeltas },
    perEval: evalDeltas,
  ...
  if (ctlR && txR) {
    lines.push(drow('Stage 1 OK',                ctlR.stage1.ok,           ctlR.stage1.okPct,           txR.stage1.ok,           txR.stage1.okPct));
    lines.push(drow('Variant activated',         ctlR.variantCapabilityLoaded,      ctlR.variantCapabilityLoadedPct,      txR.variantCapabilityLoaded,      txR.variantCapabilityLoadedPct));
    lines.push(drow('Gate valid',                ctlR.gateValid,           ctlR.gateValidPct,           txR.gateValid,           txR.gateValidPct));
    lines.push(drow('Artifact materialized',     ctlR.skillMaterialized,   ctlR.skillMaterializedPct,   txR.skillMaterialized,   txR.skillMaterializedPct));
    lines.push(drow('Stage 2 attempted',         ctlR.stage2.attempted,    ctlR.stage2.attemptedPct,    txR.stage2.attempted,    txR.stage2.attemptedPct));
    lines.push(drow('Stage 2 OK',                ctlR.stage2.ok,           ctlR.stage2.okPct,           txR.stage2.ok,           txR.stage2.okPct));
    lines.push('');
    lines.push(`Mean Stage 1 latency:   control ${fmtMs(ctlR.stage1.meanMs)}   variant ${fmtMs(txR.stage1.meanMs)}`);
    lines.push(`Mean Stage 2 latency:   control ${fmtMs(ctlR.stage2.meanMs)}   variant ${fmtMs(txR.stage2.meanMs)}`);
    if (txR.skillBody.meanBytes != null) {
