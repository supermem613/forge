...
export function buildDispatchPrompt({ controlRun, variantRun, variantName, refitStale }) {
  ...
  const lines = [];
  lines.push(`# Judge sub-agent prompt — DISPATCH WITH \`model: ${REQUIRED_JUDGE_MODEL}\``);
  lines.push('');
  ...
  lines.push(`- Wrong \`model\` value (anything other than \`${REQUIRED_JUDGE_MODEL}\` or the auto-fail sentinel).`);
  lines.push('');
  ...
  lines.push('');
  lines.push('Reply with a one-line summary: `wrote N verdicts under <runDir>/judge-verdicts/`. The orchestrator will run `forge judge <experiment> --mode validate` to catch any wrong-shape verdict early, then `--mode collect` (which BLOCKS with a non-zero exit if any file is missing or invalid) before `forge score`.');
  return lines.join('\n');
}
