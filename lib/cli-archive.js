...
import { zipDirContents } from './zip.js';
...
// The zip is written in-process by lib/zip.js so the step never depends on
// an external shell or a PowerShell module load.
...
export async function zipAllRuns(rootDir) {
  ...
  for (const v of variants) {
    ...
    for (const r of runs) {
      ...
      const { files: zippedCount } = await zipDirContents(runDir, zipPath);
      const zipSize = (await fs.stat(zipPath)).size;
      // Integrity gate. Every source file must appear as a zip entry before
      // the run dir is removed. The in-process writer reports its own entry
      // count, so a mismatch means the zip is incomplete and the run dir is
      // kept intact for a retry.
      if (zippedCount !== srcCount) {
        await fs.rm(zipPath, { force: true });
        throw new Error(`archive: zip entry count ${zippedCount} != ${srcCount} source files for ${runDir}`);
      }
